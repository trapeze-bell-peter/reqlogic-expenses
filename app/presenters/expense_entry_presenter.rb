...
class ExpenseEntryPresenter < BasePresenter
  ...
  def unit_cost
    expense_entry_form.number_field :unit_cost, class: 'form-control', required: true, min: '0.01', step: '0.01',
                                                data: { action: 'keyup->expenses#recalcClaim change->expenses#recalcClaim',
  ...
  def delete_button
    if expense_entry.persisted?
      view.link_to(view.expense_entry_path(expense_entry),
                   method: :delete, remote: true, data: { action: 'ajax:success->expenses#deleteRow' }, &method(:delete_icon))
    else
      view.link_to('_', href: '#', data: { action: 'click->expenses#deleteRow' }, &method(:delete_icon))
    end
  end

  def insert_button(&block)
    view.link_to(view.new_expense_entry_path(expense_claim_id: expense_entry.expense_claim_id),
                 data: { action: 'click->expenses#insertRow' }) do
